...
contract("FeatureFeeManager", function(accounts) {
    ...
    before("setup", function(done) {
        ...
        .then(() => TIME.approve(timeHolderWallet, CreateWalletFeatureRequiredBalance, {from: timeHolder1}))
        .then(() => timeHolder.deposit(CreateWalletFeatureRequiredBalance, {from : timeHolder1}))
    ...
    it("should allow to execute WalletsManager#createWallet() account with deposit > feature_price", async () => {
        ...
        .then(() => walletsManager.createWallet.call([timeHolder1, timeHolder2], 1, "Test Wallet 1", {from :timeHolder1}))
        .then(r => assert.equal(r, ErrorsEnum.OK))
        .then(() => walletsManager.createWallet([timeHolder1, timeHolder2], 1, "Test Wallet 1", {from :timeHolder1}))
    ...
    it("should not allow to execute WalletsManager#createWallet() account with deposit < feature_price", async () => {
        ...
        .then(() => walletsManager.createWallet.call([timeHolder1, timeHolder2], 1, "Test Wallet 1", {from :timeHolder4}))
        .then(r => assert.equal(r, ErrorsEnum.FEATURE_IS_UNAVAILABE))
        .then(() => walletsManager.createWallet([timeHolder1, timeHolder2], 1, "Test Wallet 1", {from :timeHolder4}))
        .then(tx => eventsHelper.extractEvents(tx, "WalletCreated"))
