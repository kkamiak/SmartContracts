...
contract('LOC Manager', function(accounts) {
    ...
    context("with five CBE keys", function() {
        ...
        it("should show 0 LHT balance", function () {
            return Setup.assetsManager.getAssetBalance.call(SYMBOL2).then(function (r) {
                assert.equal(r, 0);
            });
        });
        ...
        it("should have LOCManager as an owner of LHT asset", function () {
            return Setup.assetsManager.getAssetsForOwner.call(Setup.chronoMint.address).then(_assets => {
                console.log("Assets for LOC: " + JSON.stringify(_assets, null, 2));

                return Setup.erc20Manager.getTokenAddressBySymbol.call(SYMBOL2).then(_lhtAddress => {
                    console.log("LHT address: " + _lhtAddress);
                })
                .then(() => Setup.assetsPlatformRegistry.getAssetOwners.call(SYMBOL2)).then(_owners => {
                    console.log("LHT owners:");
                    console.log(_owners);
                    console.log("LOCManager address:");
                    console.log(Setup.chronoMint.address);
                })
            })
            .then(() => {
                return Setup.assetsPlatformRegistry.isAssetOwner.call(SYMBOL2, Setup.chronoMint.address).then(_isOwner => {
        ...
        it("should show 1% of transferred to exchange 500000 on rewards contract balance", function () {
            return tokenContractBySymbol(SYMBOL2, ChronoBankAssetWithFeeProxy).then(_assetWithFeeProxy => {
                ...
                .then(_assetWithFee => {
                    console.log(_assetWithFee.address);
                    console.log(_assetWithFeeProxy.address);
                    return _assetWithFee.feeAddress.call().then(_feeAddress => assert.equal(_feeAddress, Setup.rewards.address))
                })
                .then(() => {
                    return _assetWithFeeProxy.balanceOf.call(Setup.rewards.address).then(function (r2) {
                        console.log(r2);
        ...
        it("should be able to send 100 TIME to owner", function () {
            return Setup.assetsManager.sendAsset.call(SYMBOL, owner, 100).then(function (r) {
                return Setup.assetsManager.sendAsset(SYMBOL, owner, 100, {
                    from: accounts[0],
                    gas: 3000000
                }).then(function () {
                    assert.isOk(r);
        ...
        it("check Owner has 100 TIME", function () {
            return tokenContractBySymbol(SYMBOL, ChronoBankAssetWithFeeProxy).then(_assetWithFeeProxy => {
                return _assetWithFeeProxy.balanceOf.call(owner).then(function (r2) {
                    assert.equal(r2, 100);
                })
            })
        })
        ...
        it("owner should be able to approve 100 TIME to TimeHolder", function () {
            return Setup.timeHolder.wallet.call().then(_walletAddress => {
                return tokenContractBySymbol(SYMBOL, ChronoBankAssetProxy).then(_assetProxy => {
                    return _assetProxy.approve.call(_walletAddress, 100, {from: owner}).then((r) => {
                        return _assetProxy.approve(_walletAddress, 100, {from: owner}).then(() => {
                            assert.isOk(r);
                        })
                    })
                })
            ...
            return Setup.rewards.closePeriod({from: owner}).then(() => {
                return Setup.rewards.depositBalanceInPeriod.call(owner, 0, {from: owner}).then((r1) => {
                    return Setup.rewards.totalDepositInPeriod.call(0, {from: owner}).then((r2) => {
                        return tokenContractBySymbol(SYMBOL2, ChronoBankAssetWithFeeProxy).then(_assetWithFeeProxy => {
                            return Setup.rewards.rewardsFor.call(_assetWithFeeProxy.address, owner).then((r3) => {
                                return Setup.rewards.withdrawReward(_assetWithFeeProxy.address, r3).then(() => {
                                    return _assetWithFeeProxy.balanceOf.call(owner).then((r4) => {
                                        assert.equal(r1, 100);
                                        assert.equal(r2, 100);
                                        console.log(r3);
